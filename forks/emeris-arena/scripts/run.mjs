/**
 * One button: start the arena, open a chromeless window, leave nothing behind.
 * Single process â€” Vite runs in here, so there is nothing to spawn or reap.
 *
 * Browser-host launch layer â€” disposable. If the host stops being a browser,
...
const quiet = process.env.ARENA_NO_OPEN === "1" || process.env.MEADOW_NO_OPEN === "1";
...
async function ours() {
  try {
    ...
    return (info?.arena === true || info?.meadow === true) && info?.root === root;
  } catch {
    return false;
  }
}
...
/** Only used when something foreign squats the port â€” never on the fast path. */
function freePort() {
  try {
